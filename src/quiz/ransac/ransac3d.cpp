...
std::unordered_set<int> Ransac(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
	...
	while(maxIterations--){
		...
		for(int index = 0; index < cloud->points.size(); index++)
		{
			...
			//float distance = fabs((p1.y - p2.y)*sample.x + (p2.x - p1.x)*sample.y + (p1.x*p2.y - p2.x*p1.y))/sqrt(pow(p1.y - p2.y,2) + pow(p2.x - p1.x,2));
		...
		if(cnt > _max){
			...
			_max = cnt;
			cout<<_max<<endl;
			// cout<<max_p1.x<<' '<<max_p1.y<<endl;
			// cout<<max_p2.x<<' '<<max_p2.y<<endl;
		}
	}
	...
	for(int index = 0; index < cloud->points.size();index++){
		pcl::PointXYZ sample = cloud->points[index];
		//float distance = fabs((max_p1.y - max_p2.y)*sample.x + (max_p2.x - max_p1.x)*sample.y + (max_p1.x * max_p2.y - max_p2.x * max_p1.y))/sqrt(pow(max_p1.y - max_p2.y,2) + pow(max_p2.x - max_p1.x,2));
		float distance3D = fabs(i*sample.x + j*sample.y + k*sample.z + D) / sqrt(pow(i,2) + pow(j,2) + pow(k,2));
...
int main ()
{
	...
	// Create data
	//ppcl::PointCloud<pcl::PointXYZ>::Ptr cloud = CreateData();
